...
c = conn.cursor()
c.execute('CREATE TABLE IF NOT EXISTS capsFair(deviceID REAL, deviceIP TEXT, temperature REAL, humidity REAL, ozone REAL, CO REAL, NO2 REAL, NH3 REAL, date TEXT, latitude REAL, longitude REAL, altitude REAL)' )
lock = threading.Lock()
...
class MyTCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        while True:
            ...
            if command == 68:
                ...
                value_buf = value
                self.put_database(deviceID, deviceIP, date, value, latitude, longitude, altitude)
            elif data[0] == 44:
                location_data = data.decode().split(",")
                latitude = location_data[1]
                longitude = location_data[2]
                altitude = location_data[3]
                self.put_database(deviceID, deviceIP, date, value_buf, latitude, longitude, altitude)

                """latitude = str2num(location_data[0])
                print(latitude)
                longitude = str2num(location_data[1])
                print(longitude)
                altitude = str2num(location_data[2])
                print(altitude)"""
                
                """if data[15] == 'G':
                    if data[16]:
                        gps_string = data[16:]
                        gps_values = gps_string.split(",")
                        latitude = float(gps_values[0])
                        longitude = float(gps_values[1])
                        altitude = float(gps_values[2])
                    else:
                        self.location = self.request.recv(1024)
                        location_data = self.location.split(",")
    ...
    def put_database(self, id, ip, date, r, latitude, longitude, altitude):

        try:
            lock.acquire(True)
            if latitude == 0 and longitude == 0 and altitude == 0:
                c.execute(
                    "INSERT INTO capsFair(deviceID, deviceIP, temperature, humidity, ozone, CO, NO2, NH3, date) VALUES(?,?,?,?,?,?,?,?,?)",
                    (id, ip, r[0], r[1], r[2], r[3], r[4], r[5], date,))
                conn.commit()
            else:
                c.execute(
                    "INSERT INTO capsFair(deviceID, deviceIP, temperature, humidity, ozone, CO, NO2, NH3, date, latitude, longitude, altitude) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
                    (id, ip, r[0], r[1], r[2], r[3], r[4], r[5], date, latitude, longitude, altitude))
                conn.commit()  
        finally:
            lock.release()
